...
@pytest.mark.calc_method('bshr')
@pytest.mark.i_calc_method('bit_shift_right')
class TestBshr:
    @staticmethod
    def bin(number):
    ...
    @pytest.mark.parametrize('a', tIG.ints(False, True))
    ...
    @pytest.mark.parametrize('b', tIG.robust_ints(case_num=20, negatives=False, zero=True))
